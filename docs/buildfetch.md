...
Moon reads the token from the `BUILDFETCH_TOKEN` environment variable. The
local cache is read-only, so local builds can consume cached artifacts without
uploading new artifacts to BuildFetch.
